var serviceAccount = require("./loan-calculator-7f21f-firebase-adminsdk-x611v-58adf47a13.json");
...
const firebase = admin.initializeApp({
  ...
  databaseURL: "https://loan-calculator-7f21f.firebaseio.com"
});
...
app.get("/", (req, res) => {
	res.send(`LOAN CALCULATOR : Hello from Express in Cloud Functions for Firebase`);
});

app.get("/test", (req, res) => {
var cityRef = db.collection('loancalc.db').doc('4ePNX9ZrsPxYxtE3DKmu');
var getDoc = cityRef.get()
    .then(doc => {
      if (!doc.exists) {
        var aor =('No such document!');
      } else {
        aor=(doc.data());
      }
    })
    .catch(err => {
      aor=('Error getting document', err);
    });
    var test = {
    	aor: aor,
    }
	...
	/*var aor = db.collection("loancalc.db").doc("4ePNX9ZrsPxYxtE3DKmu").get().then(snapshot => {
	snapshot.exists ? res.json(201, snapshot.data()) : res.json(500, { message: "doc not found" });
	})*/
});
...
app.get("/:claster/:price/:dp/:term/:discount", (req, res) => {
	...
	switch(claster)	{
		    ...
		    case 'CebuSpclNoIns' :
						...
						getpercent = Math.round(percent * 100).toFixed(2);
						totalfin = (((Number(price)) + processfee - (Number(dp)))+insurance).toFixed(2);
						aor = db.collection("loancalc.db").doc("4ePNX9ZrsPxYxtE3DKmu").get().then(snapshot => {
						snapshot.exists ? res.json(201, snapshot.data()) : res.json(500, { message: "doc not found" });
						})
						//aor = 1.92;
					ma = Math.ceil((totalfin * (1+((aor/100)*(Number(term)))))/(Number(term))).toFixed(2);
	...
	//var setDoc = db.collection('loancalc.db').doc(i).dset(details).then(() => {
